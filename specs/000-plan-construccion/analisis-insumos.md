...
| H-11 | Prorrateo por grupo de unidades | El motor de `003-liquidacion` reparte **un solo coeficiente** sobre todo el padrón, con dos subtotales por naturaleza (RN-05). Muchos consorcios reparten ciertos rubros sólo entre un grupo —el portón y la limpieza del subsuelo entre las cocheras, el ascensor desde el primer piso—, y eso no está en el punto 7 ni en la especificación de la iteración 2 | Preguntarle al cliente **antes** de escribir el motor si Grupo Delta reparte algún rubro por grupo, y con qué criterio. Si la respuesta es que sí, es un cambio de modelo (`RubroGasto` con alcance, o coeficientes por grupo), no un parche posterior. **Resuelto el 2026-09-10 para la iteración 2**: el prorrateo va sobre el padrón completo (`003-liquidacion` FR-006b). La suposición cae si las tres liquidaciones reales muestran lo contrario |

Nota sobre H-11: sale de una consulta del 2026-09-10, al agregar el tipo de unidad. Es el único hueco
que apareció después de cerrada la iteración 1, y el único que bloquea la 2 en su diseño y no en sus
insumos: reparto por grupo cambia la firma del motor de prorrateo, así que la pregunta va antes de la
primera prueba, no después.
