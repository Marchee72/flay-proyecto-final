...
4. `docker run` idempotente `flay-db` (`pgvector/pgvector:0.8.6-pg18`); `.env.example` (+`SHADOW_DATABASE_URL`) y `.env.local` si no existe; `npx auth secret` nunca versionado.
