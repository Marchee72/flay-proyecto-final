...
- [x] T023 [US3] Crear flujo despliegue: verde → `db:deploy` demo + publicar; secretos desde proveedor; migrar-antes-de-servir; migración fallida bloquea + alerta (FR-019, M-09) — `.github/workflows/despliegue.yml`, probado de punta a punta el 09/09/2026: la integración a `main` migró, publicó y verificó salud sin intervención; la `version` de `/api/salud` coincide con el commit (SC-007)
- [ ] T024 [US3] Proteger `main`: sin push directo, PR + revisión del otro + verde (FR-020, SC-012) — repositorio hecho público (acta §2.3); 1 revisión aprobada, `verificar` requerido y estricto, alcanza a administradores, sin force push ni borrado; envío directo a `main` rechazado por GH006. **Protección levantada el 09/09/2026 a pedido del equipo** (ver acta §2.3): FR-020/SC-012 sin cumplir por configuración hasta restablecerla
